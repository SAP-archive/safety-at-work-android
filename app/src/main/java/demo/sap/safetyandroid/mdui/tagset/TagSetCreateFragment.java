...
import demo.sap.safetyandroid.databinding.FragmentTagsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing TagSetType entity.
 * This fragment is contained in the {@link TagSetActivity}.
 */
public class TagSetCreateFragment extends InterfacedFragment<TagSetType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(TagSetCreateFragment.class);
    ...
    /** TagSetType object and it's copy: the modifications are done on the copied object. */
    private TagSetType tagSetTypeEntity;
    private TagSetType tagSetTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentTagsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** TagSetType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for TagSet to enter values for the properties.
     ...
     *            TagSetType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.tagSetType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.tagSetType.getLocalName();
        }

        ((TagSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            tagSetTypeEntity = createTagSetType();
        } else {
            tagSetTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        TagSetType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (TagSetType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            tagSetTypeEntityCopy = (TagSetType) tagSetTypeEntity.copy();
            tagSetTypeEntityCopy.setEntityTag(tagSetTypeEntity.getEntityTag());
            tagSetTypeEntityCopy.setOldEntity(tagSetTypeEntity);
            tagSetTypeEntityCopy.setEditLink((tagSetTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            tagSetTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, tagSetTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isTagSetTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((TagSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(tagSetTypeEntityCopy);
        } else {
            viewModel.update(tagSetTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new TagSetType instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new TagSetType instance
     */
    private TagSetType createTagSetType() {
        TagSetType tagSetTypeEntity = new TagSetType(true);
        return tagSetTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<TagSetType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(tagSetTypeEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentTagsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setTagSetType(tagSetTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isTagSetTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_tagsettype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.tagSetType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<TagSetType> result) {
        String errorMessage;
