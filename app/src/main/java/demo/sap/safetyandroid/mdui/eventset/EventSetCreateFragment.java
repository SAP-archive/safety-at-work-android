...
import demo.sap.safetyandroid.databinding.FragmentEventsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing EventSetType entity.
 * This fragment is contained in the {@link EventSetActivity}.
 */
public class EventSetCreateFragment extends InterfacedFragment<EventSetType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EventSetCreateFragment.class);
    ...
    /** EventSetType object and it's copy: the modifications are done on the copied object. */
    private EventSetType eventSetTypeEntity;
    private EventSetType eventSetTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentEventsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** EventSetType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for EventSet to enter values for the properties.
     ...
     *            EventSetType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.eventSetType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.eventSetType.getLocalName();
        }

        ((EventSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            eventSetTypeEntity = createEventSetType();
        } else {
            eventSetTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        EventSetType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (EventSetType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            eventSetTypeEntityCopy = (EventSetType) eventSetTypeEntity.copy();
            eventSetTypeEntityCopy.setEntityTag(eventSetTypeEntity.getEntityTag());
            eventSetTypeEntityCopy.setOldEntity(eventSetTypeEntity);
            eventSetTypeEntityCopy.setEditLink((eventSetTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            eventSetTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, eventSetTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isEventSetTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((EventSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(eventSetTypeEntityCopy);
        } else {
            viewModel.update(eventSetTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new EventSetType instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new EventSetType instance
     */
    private EventSetType createEventSetType() {
        EventSetType eventSetTypeEntity = new EventSetType(true);
        return eventSetTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<EventSetType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(eventSetTypeEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentEventsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setEventSetType(eventSetTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isEventSetTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_eventsettype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.eventSetType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<EventSetType> result) {
        String errorMessage;
