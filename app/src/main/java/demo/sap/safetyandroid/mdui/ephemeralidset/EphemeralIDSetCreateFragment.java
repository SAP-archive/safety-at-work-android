...
import demo.sap.safetyandroid.databinding.FragmentEphemeralidsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing EphemeralIDSetType entity.
 * This fragment is contained in the {@link EphemeralIDSetActivity}.
 */
public class EphemeralIDSetCreateFragment extends InterfacedFragment<EphemeralIDSetType> {

    private static final Logger LOGGER = LoggerFactory.getLogger(EphemeralIDSetCreateFragment.class);
    ...
    /** EphemeralIDSetType object and it's copy: the modifications are done on the copied object. */
    private EphemeralIDSetType ephemeralIDSetTypeEntity;
    private EphemeralIDSetType ephemeralIDSetTypeEntityCopy;

    /** DataBinding generated class */
    private FragmentEphemeralidsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** EphemeralIDSetType ViewModel */
    ...
    /**
     * This fragment is used for both update and create for EphemeralIDSet to enter values for the properties.
     ...
     *            EphemeralIDSetType if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.ephemeralIDSetType.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.ephemeralIDSetType.getLocalName();
        }

        ((EphemeralIDSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            ephemeralIDSetTypeEntity = createEphemeralIDSetType();
        } else {
            ephemeralIDSetTypeEntity = viewModel.getSelectedEntity().getValue();
        }

        EphemeralIDSetType workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (EphemeralIDSetType)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            ephemeralIDSetTypeEntityCopy = (EphemeralIDSetType) ephemeralIDSetTypeEntity.copy();
            ephemeralIDSetTypeEntityCopy.setEntityTag(ephemeralIDSetTypeEntity.getEntityTag());
            ephemeralIDSetTypeEntityCopy.setOldEntity(ephemeralIDSetTypeEntity);
            ephemeralIDSetTypeEntityCopy.setEditLink((ephemeralIDSetTypeEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            ephemeralIDSetTypeEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, ephemeralIDSetTypeEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isEphemeralIDSetTypeValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((EphemeralIDSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(ephemeralIDSetTypeEntityCopy);
        } else {
            viewModel.update(ephemeralIDSetTypeEntityCopy);
        }
        return true;
    }

    /**
     * Create a new EphemeralIDSetType instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new EphemeralIDSetType instance
     */
    private EphemeralIDSetType createEphemeralIDSetType() {
        EphemeralIDSetType ephemeralIDSetTypeEntity = new EphemeralIDSetType(true);
        return ephemeralIDSetTypeEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<EphemeralIDSetType> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(ephemeralIDSetTypeEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentEphemeralidsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setEphemeralIDSetType(ephemeralIDSetTypeEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isEphemeralIDSetTypeValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_ephemeralidsettype);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.ephemeralIDSetType.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<EphemeralIDSetType> result) {
        String errorMessage;
